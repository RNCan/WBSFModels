...
double pc_rand_bi(int i,double *ratio_left,double *mean_left,double *mean_right,double *stdev_left, double *stdev_right,gsl_rng *rr)
{
    ...
    if (i==0)
    {
        if		(bimodal_rand < ratio_left[i])
        {
            pc_rand = mean_left[i] +  gsl_ran_gaussian(rr,stdev_left[i]);
        }
        else
        {
            pc_rand = mean_right[i] + gsl_ran_gaussian(rr,stdev_right[i]);
        }
    }
    else
    {
        if		(bimodal_rand < ratio_left[i])
        {
            pc_rand = 0 - gsl_ran_exponential(rr,stdev_left[i]);
        }
        else
        {
            pc_rand = 0 + gsl_ran_exponential(rr,stdev_left[i]);
...
double pc_prob_prop(int i,double *ratio_left,double *ratio_right,double *mean_left,double *mean_right,double *stdev_left,double *stdev_right,double jump_pc_rand)
{
    ...
    else
    {
        if (jump_pc_rand<0)
        {
            prob_prop = ratio_left[i]  * gsl_ran_exponential_pdf(-jump_pc_rand,stdev_left[i]);
        }
        else
        {
            prob_prop = ratio_right[i] * gsl_ran_exponential_pdf(jump_pc_rand,stdev_right[i]);
        }
    }
    return prob_prop;
}
...
double prior_dist(int i,double value)
{
    ...
    mean[5]=-.42;
    stdev[5]=.08;		//mean[5]=.38;	stdev[5]=.675;
    mean[6]=-.545;
    stdev[6]=.12;		//mean[6]=.285;	stdev[6]=.0725;
...
void testing_parms(void *Paramstuff)
{
    ...
    j=2;
    Params->PARS[2]=3.801894e-02;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=3;
    Params->PARS[3]=4.011745e-06;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=4;
    Params->PARS[4]=2.800667e+00;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=5;
    Params->PARS[5]=3.981072e-01;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=6;
    Params->PARS[6]=3.235937e-01;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=10;
    Params->PARS[10]=7.943282e-02;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=11;
    Params->PARS[11]=0.000000e+00;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=12;
    Params->PARS[12]=0.000000e+00;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=14;
    Params->PARS[14]=2.618589e+00;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=15;
    Params->PARS[15]=2.310000e+00;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=16;
    Params->PARS[16]=1.1e+00;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=17;
    Params->PARS[17]=5.250000e-02;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=18;
    Params->PARS[18]=2.400000e-01;
    Params->MLE[j] = Params->PARS[j];
    printf("Parm[%d]=%4.3e\n",j,Params->PARS[j]);
    j=19;
    Params->PARS[18]=2.400000e-01;
