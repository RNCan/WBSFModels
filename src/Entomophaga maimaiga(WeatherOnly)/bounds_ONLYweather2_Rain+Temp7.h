...
//else if (i==4)	{	low = 0.001;			high = 3.0;	}		//k
    ...
    {
        low = 1.01;			   //CK//  beta = the size of accumulated rainfall window
        high = 9.01;
    }
    ...
    {
        low = 0.000001;			   //sdR	RESTING SPORES
        high = 0.600;
    }
    else if (i==12)
    {
        low = 0.000001;			   //sdC  CONIDIA
        high = 0.350;
    }
...
//else if (i==19)	{	low = -2.0;			high = -0.4;	}		//CK//
    ...
    {
        low = 380.0;			   //CK// stop2 R_end.  time when resting spores stop blooming
        high = 550.0;
    }

    else if (i==20)
    {
        low = 4.0;			   //CK// Covered_R
        high = 18.50;
    }
...
//else if (i==21)	{	low = 0.01;			high = 20.0;	}		//CK// Rain scaling parameter func 3 and 4
    ...
    {
        low = -2.50;			   //CK// Rain scaling parameter func 2
        high = 0.50;
    }
//else if (i==21)	{	low = 0.0000001;			high = 0.2;	}		//CK// temp scaling parameter
    ...
    {
        low = -3.0;			   //CK// RH scaling parameter
        high = -1.0;
    }

    else if (i==23)
    {
        low = -1.50;			   //CK// temp scaling parameter for muF
        high = -0.200;
    }
//else if (i==23)	{	low = 0.0001;			high = 0.2;	}		//CK// stop2 R_end.  time when resting spores stop blooming
    ...
    {
        low = 80.0;			   //CK// R_start.  time when resting spores start blooming
        high = 150.0;
    }
...
//else if (i==28)	{	low = 0.015;			high = 0.07;	}		//CK// size_S. effect of size on susceptibility.
    ...
    {
        low = 250.0;			   //CK// size_S. effect of size on susceptibility.
        high = 350.0;
    }
    ...
    {
        printf("PROBLEM WITH BOUNDS ON PARAMETERS!!\n");
        return 0;
    }
}
// ------------------------------------------------------------------------------------ //
...
// i=zero isnt used normally
    ...
    {
        printf("PROBLEM WITH BOUNDS ON S(0) or R(0)!!\n");
        return 0;
    }
}
// --------------------------------------------------------------------------------------- //
double prior_bound(int i,int j)
{
    ...
    {
        low = 0.01;			   //CK//  beta = the size of accumulated rainfall window
        high = 20.01;
    }
    ...
    {
        low = 0.01;			   //CK// theta = delay for when to start accumulating rainfall
        high = 4.01;
    }
...
//else if (i==16)	{	low = 0;			high = 10;	}		//r_time
    ...
    {
        low = 0.1;			   //r_time  //CK// changed!		//CK// to change!
        high = 20.0;
    }
    else if (i==17)
    {
    ...
    {
        low = .01;			   //CK// dummy param for testing!!!
        high = 0.4;
    }

    else if (i==20)
    {
        low = 0.1;			   //CK// covered cage scaling parameter
        high = 5.4;
    }
//else if (i==20)	{	low = 0.00001;			high = 0.4;	}		//CK// rain scaling parameter
    ...
    {
        low = 0.00001;			   //CK// temp scaling parameter
        high = 0.4;
    }

    else if (i==22)
    {
        low = 0.00001;			   //CK// RH scaling parameter
        high = 0.4;
    }

    else if (i==23)
    {
        low = 0.0000001;			   //CK// Average R(0) for all populations
        high = 0.5;
    }
...
//else if (i==26)	{	low = 1E-6;			high = 0.5;	}		//CK// scaler for TEMP affect of muF func 2
    ...
    {
        low = 0.01;			   //CK// scaler for TEMP affect of muF func 3 and 4
        high = 20.0;
    }
    else if (i==27)
    {
        low = 1E-6;			   //CK// scaler for TEMP affect of muF func 2
        high = 0.5;
    }
...
//ulow=0;	uhigh=0;	mean=0;	sdev=0;	Slow=0;	Shigh=0; Rmean=0; Rsdev=0;
    ...
    else if ((i>20 && i<=30+DATA_SETS) || (i>50 && i<=50+DATA_SETS))
    {
        ...
        {
            printf("called index that is not a data-set!!\n");
        }
    }
    else
    {
    ...
    {
        printf("PROBLEM WITH MEAN or SDEV in GAUSSIAN PARAMETERES!!\n");
        return 0;
    }
}
...
double fixed_patch(int i,int j)
{
    ...
    {
        printf("i=%d\t j=%d\t PROBLEM WITH INDEX ON S(0) or R(0)!!\n",i,j);
        getc(stdin);
        return 0;
    }
    ...
    {
        printf("PROBLEM WITH CHOOSING S(0) or R(0)!!\n");
        return 0;
    }
...
double start_pc(int i)
{
    ...
    if (i==0)	pc_value= 5.186043e+00;
    else if (i==1)	pc_value= 1.601133e+00;
    else if (i==2)	pc_value= 6.347635e-01;
    else if (i==3)	pc_value= .373140e+00;
    else if (i==4)	pc_value=-5.457952e-01;
    else if (i==5)	pc_value= 2.441176e-01;
    else if (i==6)	pc_value= 4.549583e-01;
    else if (i==7)	pc_value= 4.288782e-01;
    else if (i==8)	pc_value=-1.874400e-01;
    else if (i==9)	pc_value= 2.059179e-01;
    else if (i==10)	pc_value=-2.302962e-01;
    else if (i==11)	pc_value=-2.235796e-01;
    else if (i==12)	pc_value= 1.475449e-01;
    else if (i==13)	pc_value=-1.769097e-01;
    else if (i==14)	pc_value=-1.212636e-01;
    else if (i==15)	pc_value=-1.272351e-01;
    else if (i==16)	pc_value= 4.185948e-01;
    else if (i==17)	pc_value=-1.060853e-01;
    else if (i==18)	pc_value=-3.476973e-01;
    else if (i==19)	pc_value= 9.517782e-01;
    else if (i==20)	pc_value= 2.936443e-01;
    else if (i==21)	pc_value= 1.013198e-01;
    else if (i==22)	pc_value= 8.026273e-01;
    else if (i==23)	pc_value=-1.248938e-01;
    else if (i==24)	pc_value= 6.162489e-01;
    else if (i==25)	pc_value= 7.884241e-01;
    else if (i==26)	pc_value= 4.769538e-01;
    else if (i==27)	pc_value=-8.913772e-01;
    else if (i==28)	pc_value= 1.910217e-01;
    else if (i==29)	pc_value= 5.366982e-01;
    else if (i==30)	pc_value=-3.458366e-01;
    else if (i==31)	pc_value=-2.682097e-01;
    else if (i==32)	pc_value= 7.909581e-01;
    else if (i==33)	pc_value=-3.420851e-01;
    else if (i==34)	pc_value= 2.193495e-02;
    else if (i==35)	pc_value= 1.685945e-01;
...
void parm_range_inc(void *Paramstuff,double parm_inc,double host_inc,double initR_inc,int num_adj_pars)
{
    ...
    if (num_adj_pars>0)
    {
        ...
        {
            Params->parm_low[i]  = bound(i,1);
            Params->parm_high[i] = bound(i,2);
...
// ----------------------------- host/initR increments and end of spore germination -------------------------------- //
    for (i=0; i<=DATA_SETS; i++)
    {
        ...
        Params->parm_low[40+i]  = patch_bound(i,1);
        Params->parm_high[40+i]= patch_bound(i,2);	//CK// Site-Specific muF
        Params->parm_low[50+i]  = patch_bound(i,3);
        Params->parm_high[50+i]= patch_bound(i,4);	// R(0)
...
void random_restart_parms(void *Paramstuff,gsl_rng *r_seed,int num_adj_pars,int test,int pro)
{
    ...
    for (i=1; i<=num_adj_pars; i++)
    {
        if ((i==2||i==3||i==4||i==5||i==6||i==7||i==9||i==10||i==11||i==12||i==13||i==14||i==15||i==16||i==17||i==18||i==19||i==20||i==21||i==22||i==23||i==24||i==25||i==26||i==27||i==28||i==29) && (i!=pro))
        {
            ...
            {
                printf("in bounds.h random_restart_parms: bad initial parameter!!!\n");
                getc(stdin);
            }
