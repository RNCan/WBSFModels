...
#define MAX_WEEKS2 8000000	// larger than the number of lines in the climate change data set
	...
	int j;
	int q;
	int num_weeks1[DATA_SETS + 1];			// used for output to file
	...
	//    char *file_name3="wonly_highRCP_END_BC_s25_withnorth_hdefof_0.05";
	//const char* file_name3 = "Michigan";
	const char* file_name3 = "Zone(150x132)";
	//////////////////////////////////////////////////////////////////////////////////////////////
	char* file_type = ".txt";
	...
	for (j = 1; j <= DATA_SETS; j++)
	{
		...
		{
			Params->EXPDATA[j][i][0] = Sdata[i];
			Params->EXPDATA[j][i][1] = Fdata[i];
			Params->EXPDATA[j][i][2] = Ddata[i];
			Params->EXPDATA[j][i][3] = Cdata[i];
			Params->EXPDATA[j][i][4] = D2data[i];
	...
	for (j = 1; j <= DATA_SETS; j++)
	{
		...
		while (fscanf(ftp_data, "%lf %lf %lf %lf \n", &rain, &minRH, &maxT, &aveT) != EOF)
		{
			...
			if (i == 7227000)
				//if (i == 365)//test with only one point
			{
				weeks = 0;	   //JL: Whole gypsy moth range; 132*150 grids *365 days=7227000 lines for each year.
				i = 0;
				FlagF = 0;
				q++;
				printf(".");
			...
			Params->CCDATA[q][i][0] = rain;
			Params->CCDATA[q][i][1] = minRH;
			Params->CCDATA[q][i][2] = maxT;
			Params->CCDATA[q][i][3] = aveT;
	...
	for (j = 1; j <= DATA_SETS; j++)
	{
		fprintf(fp_weeks, "%d\t", num_weeks1[j]);
	}
	fclose(fp_weeks);
