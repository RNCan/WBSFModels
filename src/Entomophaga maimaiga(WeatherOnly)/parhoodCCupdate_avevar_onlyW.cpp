...
// --------------------------------------- Name for Output Files ----------------------------------------------------- //
    char strFileName[256] = { 0 };					// from filenames.h; name for the output file of mean of fraction infected at each location
    GetString(pro,0,strFileName,255);
    fflush(stdout);		//getc(stdin);
    FILE *fp_results;

    char strFileNameV[256] = { 0 };  //Name for the output file of variance of fraction infected at each location
    GetStringV(pro,0,strFileNameV,255);
    fflush(stdout);		//getc(stdin);
    FILE *fp_results_v;
// ---------------------------------------- Random Number Stuff ------------------------------------------------------ //
    gsl_rng *r_seed;
    ...
    for(k=1; k<2; k++)   //which population to run; host and resting spore densities do not matter in the weather-only model
    {
        ...
        //i = 0;//Test with only one point
        {
            ...
            for(q = 0; q < 10; q++)   	//begin loop to go through 10 years of CC data FOR EACH GRID CELL!!!  QUICK AND DIRTY AVERAGES ACROSS YEARS
            {
                ...
                dim = size_t(2*MAXT3);     //CK//  holds the random variables for each day.  Need 2 per day

                //printf("MAXT3: %lf S_start: %lf\t S_end: %lf\n", MAXT3, S_start-i*365, S_end-i*365);		//getc(stdin);

                for(j=0; j<reps; j++)
                {
                    DDEVF(&Params,r_seed,dim,pop,int(MAXT3), S_start, q);   //NEED TO ADD q TO THE DDEVF FUNCTION.  UPDATE ALL WEATHER READING LINES

                    //if (pop==1){
                   // fprintf(fp,"%e\t",1-Params.survive/Params.PARS[30+pop]);//By RSA don't save this
                    //}
                }
